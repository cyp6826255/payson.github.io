...
4.通知方式-notify、
5.分发方式-publish
op0=>operation: 构建前：第三方APIKEY配置(蒲公英分发、通知机器人、AppStore)
op1=>operation: 构建前：设置构建版本号自增（以日期.构建方式.构建号自增：20231108.2.60）
op2=>operation: 构建前：获取工程配置（获取plist参数）
op3=>operation: 构建中：环境变量设置
op4=>operation: 构建中：执行Pod依赖安装
op5=>operation: 构建中：执行构建
op6=>operation: 构建中：执行归档
op7=>operation: 构建中：执行分发
op8=>operation: 构建中：执行通知
cond=>condition: Yes or No?
op_err=>operation:执行失败通知
e=>end

st->op->op0->op1->op2->op3->op4->op5->op6->op7->op8->e
cond(yes)->e
cond(no)->op_err
```
~~~
